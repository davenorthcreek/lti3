...
public class JDBC extends DataConnector {
	 ...
	 */
	  ...
	  public boolean saveToolConsumer(ToolConsumer consumer) {
	    ...
	    try {
	      ...
	      stmt.setTimestamp(n + 10, new Timestamp(from.getMillis()));
	      stmt.setTimestamp(n + 11, new Timestamp(until.getMillis()));
	      stmt.setDate(n + 12, new Date(last.getMillis()));
	...
	// Delete any users in contexts for this consumer
	      sql = "DELETE FROM " + this.prefix + DataConnector.USER_RESULT_TABLE_NAME + " WHERE consumer_key = ?";
	      stmt = this.conn.prepareStatement(sql);
	      stmt.setString(1, consumer.getKey());
	      stmt.executeUpdate();
	 ...
	 */
	  ...
	  public boolean loadResourceLink(ResourceLink resourceLink) {
	    ...
	    try {
	      ...
	      ok = rs.next();
	      if (ok) {
	        resourceLink.setContextId(rs.getString("lti_context_id"));
	        ...
	        if ((settingsValue != null) && (settingsValue.length() > 0)) {
	          try {
	            ...
	            resourceLink.setSettingsMap(settings);
	        ...
	        resourceLink.setShareApproved(false);
...
//	    settingsValue = serialize(context.settings);
	    ...
	    Map<String,List<String>> settings = resourceLink.getSettings();
	    if (settings.size() > 0) {
	      settingsValue = gson.toJson(settings);
	    }
	    int shareApproved = 0;
	    if (resourceLink.isShareApproved()) {
	      shareApproved = 1;
	    }
	    Timestamp time = new Timestamp(DateTime.now().getMillis());
	    String sql;
	    ...
	    try {
	      ...
	      stmt.setString(n + 1, resourceLink.getContextId());
	 ...
	 */
	  ...
	  public Map<String,User> getUserResultSourcedIDsResourceLink(ResourceLink resourceLink, boolean localOnly, int scope) {
	    ...
	    try {
	      ...
	      ResultSet rs = stmt.executeQuery();
	      while (rs.next()) {
	        User user = User.fromResourceLink(resourceLink, rs.getString("user_id"));
	...
	// Load the nonce
	      ...
	      stmt = this.conn.prepareStatement(sql);
	      stmt.setString(1, nonce.getConsumer().getKey());
	...
	// Load share key
	      ...
	      if (ok) {
	        ...
	        shareKey.setResourceLinkId(rs.getString("primary_context_id"));
	 ...
	 */
	  ...
	  public boolean saveResourceLinkShareKey(ResourceLinkShareKey shareKey) {
	    ...
	    try {
	      ...
	      stmt.setString(3, shareKey.getResourceLinkId());
	      stmt.setInt(4, approve);
	      stmt.setTimestamp(5, new Timestamp(shareKey.getExpires().getMillis()));
	      ok = stmt.executeUpdate() == 1;
	    } catch (SQLException e) {
	      ok = false;
	    }
	 ...
	 */
	  ...
	  public boolean saveUser(User user) {
	    ...
	    if (user.getCreated() == null) {
	      sql = "INSERT INTO " + this.prefix + DataConnector.USER_RESULT_TABLE_NAME + " (consumer_key, context_id, " +
